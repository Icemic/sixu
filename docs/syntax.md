...
**引号包裹的文本**（双引号、单引号或反引号模板字符串）后面可以加上 `#` 标记来表示该行所需的一些特殊处理，如换行、等待点击等。

后缀标记**只允许出现在引号文本之后**。普通裸文本中的 `#` 符号会作为普通字符处理，不会被解析为后缀标记。
...
条件表达式必须使用引号包裹（双引号或单引号均可），其内容将在运行时由引擎的 `eval_condition()` 异步方法求值。
...
另外，sixu 保留了一种供工具生成的特殊单行注释 directive：
