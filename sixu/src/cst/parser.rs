...
fn parse_arguments_parenthesized(
    input: Span,
) -> ParseResult<(Vec<CstArgument>, CommandSyntax)> {
...
fn parse_argument(input: Span) -> ParseResult<CstArgument> {
    ...
    // 可选的 = 和值
    let (input, equals_and_value) = opt((
        preceded(space0, tag("=")),
        preceded(space0, parse_value),
    )).parse(input)?;
    
    let (equals_token, value) = if let Some((eq, val)) = equals_and_value {
        let eq_span = SpanInfo::from_span_and_len(
            Span::new(eq.fragment()),
            1,
        );
...
fn parse_number_value(input: Span) -> ParseResult<CstValue> {
    ...
    let parsed = if raw.contains('.') {
        // 浮点数
        format::RValue::Literal(format::Literal::Float(
...
fn parse_variable_value(input: Span) -> ParseResult<CstValue> {
    ...
    let (input, var_str) = recognize(many1(alt((alphanumeric1, tag("."), tag("_"))))).parse(input)?;
...
mod tests {
    ...
    fn test_parse_tolerant() {
        ...
        // 统计命令数量
        let cmd_count = cst.nodes.iter().filter(|n| matches!(n, CstNode::Command(_))).count();
        ...
        if looks_like_command {
            match parse_command(remaining) {
                ...
                Err(nom::Err::Error(e)) | Err(nom::Err::Failure(e)) => {
                    ...
                    let (rest, _) = take::<usize, Span, nom::error::Error<Span>>(bytes_to_skip)(remaining)
...
fn parse_leading_bare(i: Span) -> ParseResult<CstLeadingTextContent> {
    ...
    Ok((i, CstLeadingTextContent::Text(text.fragment().trim().to_string())))
}
...
fn parse_quoted_string(input: Span) -> ParseResult<String> {
    let quote_char = input.fragment().chars().next()
                    ...
                    if rest.fragment().starts_with('{') {
                        ...
                        while hex_len < chars.len() && chars[hex_len] != '}' {
                            if !chars[hex_len].is_ascii_hexdigit() {
                                return Err(nom::Err::Error(nom::error::Error::new(rest, nom::error::ErrorKind::HexDigit)));
                            }
                            hex_len += 1;
                        }
    ...
    // 未闭合的字符串
    Err(nom::Err::Error(nom::error::Error::new(remaining, nom::error::ErrorKind::Tag)))
}
...
mod phase2_tests {
    ...
    fn test_parse_block_with_commands() {
        ...
        // 统计命令数量（忽略 trivia）
        let cmd_count = block.children.iter()
    ...
    fn test_parse_block_nested() {
        ...
        // 应该包含嵌套块
        let has_nested_block = block.children.iter()
